/**
  Fog-grbl-v1.0.cps
  Custom grbl post processor for Fog's CNC machine.
  Based on the Autodesk grbl post processor.
  Customized with Claude on 2026-03-07:
    - Safe retracts default changed to G28 (max Z)
    - XY homing on program end disabled
    - Z retracts to max first, then M5 + 6-second dwell so the bit
      fully stops at the top before the tool change position move
...
minimumCircularRadius = spatial(1.27, MM);
...
function writeProgramEnd() {
  ...
  onDwell(6.0); // wait for spindle to fully stop before tool change move
